# Hifz Companion — hosted HD edition (v1.23.1)
...
## A web app manifest, and an Android wrapper (v1.23.1)

The app had no web app manifest, so "Add to Home screen" produced a bookmark rather than a
standalone app. It now ships `manifest.webmanifest` and `icons/` beside `index.html`:

```
manifest.webmanifest   name, short_name, standalone display, scope, theme and background colour
icons/icon-192.png     the HC monogram
icons/icon-512.png
icons/icon-512-maskable.png   inside the 80% safe zone on the brand ground, for Android's crop
```

Verified against Chrome's installability criteria in a headless browser: manifest linked, name,
short_name, `display: standalone`, `start_url`, all three icons serving 200, service worker
present — nine of nine.

The service worker caches the manifest and its icons **one at a time, ignoring failures**, rather
than adding them to `SHELL_URLS`: `addAll()` is atomic, so one missing icon would abort the
install of the app itself.

**Upload `manifest.webmanifest` and the `icons/` folder alongside `index.html`.** Without them the
link 404s and the app is no worse than before, but nothing is gained either.

### The Android project

`hifz-companion-android-twa.zip` is a complete, ready-to-build **Trusted Web Activity**: a thin
wrapper that opens the live site full-screen in the device's own Chrome engine. The APK is a few
hundred KB, contains no Qur'anic text, shares the site's origin and stored record, and — the
reason to prefer it here — **updates reach users when the website is redeployed**, with no new
Play release per version.

It could not be compiled in the build sandbox: `dl.google.com`, `maven.google.com` and
`services.gradle.org` are refused by the egress policy, and an APK needs `android.jar`, `aapt2`,
`d8` and `apksigner` from those servers. `BUILD-THE-APK.md` inside the zip has the ten-minute
route through Android Studio, the Play checklist, and the `assetlinks.json` template — which must
be served from the **domain root**, `https://mazhar17.github.io/.well-known/assetlinks.json`, not
from `/hc/`.

No signing key was generated. A key that has travelled through a chat is not a key worth having.
